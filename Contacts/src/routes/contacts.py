...
async def read_contacts(filter: str = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts(filter, skip, limit, db)
    return contacts
...
async def read_contact(contact_id: int, db: Session = Depends(get_db)):
    ...
    if contact is None:
        raise HTTPException(status_code=status.HTTP_418_IM_A_TEAPOT, detail="Contact not found")
    return contact


@router.get("/birthdays/", response_model=List[ContactResponse])
async def retrieve_birthdays(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_by_birthdays(skip, limit, db)
    return contacts
